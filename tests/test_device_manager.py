...
class TestDeviceManager(unittest.TestCase):
    ...
    @patch('core.dlna_device.DLNADevice.play')
    def test_auto_play_video(self, mock_play):
        ...
        # Verify the play method was called
        self.assertTrue(result)
        mock_play.assert_called_once_with(video_path, True)
        
        # Verify the video was assigned to the device
        with self.device_manager.assigned_videos_lock:
            self.assertIn("TestDevice1", self.device_manager.assigned_videos)
            self.assertEqual(self.device_manager.assigned_videos["TestDevice1"], video_path)
    
    @patch('core.dlna_device.DLNADevice.play')
    @patch('core.dlna_device.DLNADevice.stop')
    def test_auto_play_different_video(self, mock_stop, mock_play):
        ...
        # Verify the play method was called with the new video
        mock_play.assert_called_once_with(video2_path, True)
        
        # Verify the new video was assigned to the device
        with self.device_manager.assigned_videos_lock:
            self.assertEqual(self.device_manager.assigned_videos["TestDevice1"], video2_path)
